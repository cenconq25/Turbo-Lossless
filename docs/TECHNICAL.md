...
Deep dive into Turbo Lossless internals: encoding format, kernel architecture, benchmarking, and file map.

---

## Encoding: Structured 12-Bit (Split12)
...
Stored as two byte-aligned arrays -- zero GPU read amplification:
```
...
- `*.row_off.bin`: row pointers
- `*.patch_cols.bin`: column indices
- `*.patch_correct.bin`: correct BF16 values
- `*.patch_wrong.bin`: wrong BF16 (from group=0 decode)
