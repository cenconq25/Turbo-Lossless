...
def prepare_tensor(W, device="cuda:0"):
    """Build 12-bit packed format + row-grouped patches (CSR) using GPU freq sort + C packer."""
    M, K = W.shape
    n = M * K

    # GPU frequency sort — much faster than numpy for large tensors
    ...
    cb_size = _pack_lib.build_codebook_12bit(
    ...
    # Pack + build row-grouped patches (CSR format)
    ...
    num_patches = _pack_lib.pack_fixed12(
    ...
    return {
        "packed": packed[:num_words],
        "codebook": codebook,
        "row_offsets": row_offsets,
        "patch_cols": patch_cols[:num_patches],
        "patch_correct": patch_correct[:num_patches],
        "patch_wrong": patch_wrong[:num_patches],
