"""
8-Tier analysis on Mistral Large 123B (4 sampled shards).
"""
...
MODEL_DIR = "/home/ubuntu/AI/ compression/models/mistral-large-123b"
...
def main():
    print("=" * 100)
    print("Turbo Lossless: 8-Tier Analysis on Mistral Large 123B (4 shards)")
    print("=" * 100, flush=True)

    shard_paths = sorted(glob.glob(os.path.join(MODEL_DIR, "consolidated-*.safetensors")))
    ...
    print(f"\n{'=' * 100}")
    print(f"SUMMARY — Mistral Large 123B ({len(all_results)} tensors from 4 shards, {elapsed:.1f}s)")
    print(f"{'=' * 100}")
    ...
    # VRAM impact
    bf16_size = 123e9 * 2 / 1e9
    compressed_size = 123e9 * (w_8t / 8) / 1e9
    print(f"\n--- VRAM Impact (123B model) ---")
    ...
    print(f"  {'Mistral Large 123B':<25s} {w_ent:>8.2f} {w_8t:>8.2f} {16/w_8t:>8.3f}x {w_esc:>8.4f}%")

    print(f"\n{'=' * 100}")
    if 16 / w_8t >= 1.5:
        print(f"VERDICT: 8-TIER CONFIRMED on Mistral Large 123B — {16/w_8t:.3f}x compression")
    else:
        print(f"VERDICT: 8-TIER on Mistral Large 123B — {16/w_8t:.3f}x compression")
    print(f"{'=' * 100}")
